...

{
    ...
    public class MotorViborita : IMotorJuego
    {
        ...
        // — Tamaño del tablero —public int Ancho { get; } = 20;

        public int Alto { get; } = 15;


        // — Estado de la víbora —private readonly LinkedList<(int x, int y)> _cuerpo = new();

        private (int x, int y) _direccion = (1, 0); // empieza moviéndose a la derechaprivate (int x, int y) _comida;
        ...
        // Propiedades de solo lectura para la UIpublic IEnumerable<(int x, int y)> Cuerpo => _cuerpo;
...

        {
            ...
            // Víbora inicial en el centro con 3 segmentos            _cuerpo.AddFirst((Ancho / 2, Alto / 2));
...

        {
            ...
            // Colisión con paredesif (nueva.Item1 < 0 || nueva.Item1 >= Ancho ||
            ...
            // Colisión con sí mismaif (_cuerpo.Contains(nueva))
        ...
        public bool Ganado() => Puntos >= 10;   // gana al llegar a 10 puntospublic bool Perdido() => _perdido;
